...
**协调器必须在 Phase 1 需求沟通阶段使用 AskUserQuestion 一次性完成 MCP 授权确认。一次确认，全流程有效。**

**授权流程（Phase 1 执行一次）**：
1. 查阅 §2 MCP 能力速查表 → 汇总本次 run 涉及的全部 MCP 工具
2. AskUserQuestion 询问用户一次 —— 「本次设计将使用以下 MCP 工具：{工具清单}，是否全部授权？」
...
> ⚠️ **关键步骤**：需求沟通完成后，必须将用户需求固化为结构化清单，供 Phase 12 覆盖度验证！

**执行**：
```
AskUserQuestion 收集完所有需求后
    ↓
整理为结构化清单
    ↓
Write → .di/requirements-checklist.md
```
...
> 捕获时间: {ISO8601} | Phase 1

## 需求项
| # | 需求 | 类型 | 优先级 | 对应 Phase |
|---|------|------|--------|-----------|
| 1 | [具体需求描述] | 架构设计/UX设计/UI设计/审问 | HIGH/MEDIUM | [Phase] |
| 2 | ... | | | |

## 范围说明
- 任务范围: [完整设计/仅架构/仅UX/...]
- 平台限制: [Web/iOS/Android/...]
- 技术栈偏好: [如有]
- 聚焦重点: [用户最关心的维度/模块/问题]
```

> 🛑 需求项必须具体到可验证——不能写"优化UX"，要写"重构导航结构，从3层减少到2层"。
...
> ⚠️ **关键步骤**：MASTER-INDEX 闭环更新后，必须验证用户需求覆盖度！

**流程**：
```
Read .di/requirements-checklist.md → 逐项对照设计产出
    ↓
判断每项需求在产出中是否被覆盖
    ↓
覆盖度 = 已覆盖项 / 总需求项
    ↓
覆盖率 < 100% → 逐项列未覆盖项 + 原因
    → AskUserQuestion 确认处理方式
    ↓
覆盖率 = 100% → 进入完成报告
```

**逐项验证规则**：
- 架构设计需求 → 检查 `architecture-spec/` 中是否有对应章节
- UX 设计需求 → 检查 `ux-spec/` 中是否有对应内容
- UI 设计需求 → 检查 `ui-spec/` 中是否有对应设计
- 审问需求 → 检查 `design-decisions/` 中是否有对应决策记录
...
| 未覆盖需求 | 原因 | 建议 |
|-----------|------|------|
| [需求 1] | [如: 对应 Phase 未执行 / 流程中被跳过 / 超出本次范围] | [建议] |

请选择: 1.继续补充设计 2.纳入下一轮 DI 3.标注为已隐含覆盖 4.忽略
```
