...
```
协调器检测: output/{project}-analysis/
    ↓
全部5份报告存在？
├─ ✅ 是 → 富化层激活
│   1. 协调器逐一 Read 五份报告，提取关键发现
│   2. 写入 .di/context-map.md（上游富化摘要）
│   3. 告知用户："检测到 design-miner 产出，DI 专家将在独立分析基础上进行交叉印证。"
│   4. 派发专家时，prompt 中包含上游发现摘要——但专家仍先完成自身方法论，再交叉印证
│
└─ ❌ 否 → 无富化层
