...
│   2. 🔴 默认全读 01-04 标准文档——这是 design-miner 的固定产出契约
│   3. 🔴 索引表中的 05-* 自定义文档按需读取（根据本次 DI 任务范围选读）
│      例：做 UX 设计 → 读 02 + 索引中与 UX 相关的自定义文档（如 05-交互模式分析）
│      例：做架构设计 → 读 01 + 索引中与架构相关的自定义文档
│   4. 提取关键发现，写入 .di/context-map.md（上游富化摘要）
│   5. 告知用户："检测到 design-miner 产出（标准 4 份 + 自定义 N 份），DI 专家将交叉印证。"
...
**读取规则**：标准文档默认全读，自定义文档按需读。01-04 是 design-miner 的固定契约——即使某份为空也要确认。
