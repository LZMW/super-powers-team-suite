...
### 交互设计旋钮（协调器通过品味向量注入，驱动具体决策）

| 旋钮 | 1-3 | 4-7 | 8-10 |
|------|-----|-----|------|
| **INTERACTION_DENSITY** | 稀疏：单页单动作，大量留白 | 日常：2-3个主操作+辅助入口 | 密集：多快捷键、批量操作、仪表盘 |
| **NAVIGATION_DEPTH** | 扁平：≤2层，全局可见 | 标准：≤3层，面包屑导航 | 深层：>3层，需要搜索+筛选 |
| **FEEDBACK_RICHNESS** | 极简：仅必要状态切换 | 标准：loading+success+error | 丰富：骨架屏+乐观更新+触觉反馈 |
...
|---|-----------|---------|---------|---------|
| 01 | `interaction-design/01-info-architecture.md` | §信息架构 | gen-{N} | 树状结构图（标注页面类型和层级）、导航模式、层级深度 |
| 02 | `interaction-design/02-user-flows.md` | §用户流程 | gen-{N} | ≥1个核心流程（含正常路径+异常路径+决策点+边界情况） |
| 03 | `interaction-design/03-wireframes.md` | §线框图 | gen-{N} | 关键页面线框图描述（布局/元素清单/状态变化）、五态覆盖矩阵 |
| 04 | `interaction-design/04-interaction-patterns.md` | §交互模式 | gen-{N} | 交互行为定义、设计法则自检、交互反模式检查、🔬参考借鉴（如上游存在）、⚠️向协调器汇报 |
...
🚨 Guardrails:
- 禁止导航深度超过 3 层（除非 VARIANCE > 7 且刻意为之）
- 禁止同级选项超过 7±2 个（席克定律）
- 禁止全局汉堡菜单（桌面端）——除非 VARIANCE > 7 且刻意为之
- 禁止三级以上嵌套导航
- 禁止没有标注页面类型和层级的信息架构图
...
🚨 Guardrails:
- 禁止跳过空状态和错误状态——每个关键页面必须覆盖五态
- 禁止空状态只有"暂无数据"——必须含引导 CTA
- 禁止每个核心流程只有正常路径——每个流程至少含 2 个异常分支
- 禁止不标注决策点——流程中的每个分支点必须明确标注

#### 质量自检
- [ ] 是否包含 ≥1 个核心流程？
- [ ] 每个流程是否覆盖正常路径 + ≥2 个异常分支？
- [ ] 每个关键页面是否覆盖五态（正常/空/加载/错误/边界）？
- [ ] 空状态是否含引导性 CTA？
- [ ] 决策点是否明确标注？
```
...
- **元素**（从上到下/从左到右）：
  ...
  2. ...
- **状态变化**：加载→空→错误→正常 各状态表现
- **五态覆盖**：[正常态/空状态/加载态/错误态/边界态]

### 页面2: [名称]
[同上模板]

🚨 Guardrails:
- 禁止做视觉设计——线框图只用文字精确描述布局和元素，不指定颜色/字体/阴影
- 禁止假设"用户会自然理解"——每个交互必须描述触发条件和反馈
- 禁止跳过状态变化——每个页面必须覆盖五态
- 关键 CTA 必须符合费茨定律（大且靠近操作区域）
- 禁止模态弹窗套模态弹窗——失去空间上下文
- 禁止长页面无返回顶部——移动端必须含 FAB 或 sticky nav

#### 质量自检
- [ ] 线框图是否覆盖关键页面？
- [ ] 每个页面是否包含布局+元素清单+状态变化？
- [ ] 是否用文字精确描述（非视觉设计）？
- [ ] 关键 CTA 是否符合费茨定律？
- [ ] 是否存在模态弹窗嵌套？
```
...
🚨 Guardrails:
- 禁止无反馈的点击操作——每个交互必须至少含 loading→success/error 两态
- 禁止 3 个以上等宽并列卡片——至少卡片的宽高比要有变化
- 禁止假设"用户会自然理解"——每个交互决策必须追溯到 ux-research.md 中的具体用户洞察
- 禁止交互反模式检查全部"❌"——至少 1 处刻意打破常规（避免完全可预测的交互流）
...
- [ ] 交互反模式是否全部检查？
- [ ] 至少 1 处是否刻意打破常规？
- [ ] 设计决策是否可追溯到用户洞察（非"因为大家都这么做"）？
- [ ] 参考借鉴（如上游存在）：是否区分可借鉴/需规避/独创？
```
...
- **交互反模式**：全局汉堡菜单/三级嵌套/无反馈点击/模态嵌套/长页面无返回/空状态无CTA/等宽卡片
...
| 禁止模式 | 原因 | 替代方案 |
|---------|------|---------|
| 全局汉堡菜单（桌面端） | 降低可发现性 50%+ | 始终可见的导航栏 |
| 三级以上嵌套导航 | 席克定律——选项过多 | 搜索+分类筛选替代 |
| 无反馈的点击操作 | 用户不知道操作是否生效 | 至少含 loading→success/error 两态 |
| 模态弹窗套模态弹窗 | 失去空间上下文 | 内联展开或分步表单 |
| 长页面无返回顶部 | 移动端体验灾难 | FAB 或 sticky nav |
| 空状态只有"暂无数据" | 用户不知道下一步做什么 | 含引导 CTA 的空状态 |
| 3个以上等宽并列卡片 | AI 默认布局烂代码 | 至少卡片的宽高比要有变化 |

### 交互设计预制检查清单（全部通过才算完成）

- [ ] 每个核心流程覆盖正常路径 + ≥2 个异常分支
- [ ] 每个关键页面覆盖五态（正常/空/加载/错误/边界）
- [ ] 导航深度 ≤3 层，同级选项 ≤7 个
- [ ] 无全局汉堡菜单（桌面端）——除非 VARIANCE > 7 且刻意为之
- [ ] 空状态含引导性 CTA，非仅有"暂无数据"
- [ ] 关键 CTA 符合费茨定律（大且靠近操作区域）
- [ ] 无模态弹窗嵌套
- [ ] 设计决策可追溯到用户洞察（非"因为大家都这么做"）
- [ ] 至少 1 处刻意打破常规（避免完全可预测的交互流）
- [ ] 首段声明设计哲学立场（非"客观最优解"）
...
- `ui-ux-pro-max` 技能 → 检索 UX 指南（触控尺寸/动画时长/导航模式/表单反馈）/ 着陆页模式 / 反模式清单
