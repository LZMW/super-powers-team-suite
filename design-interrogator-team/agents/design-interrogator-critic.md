...
#### Phase 6b: critique-interaction/（交互审问）

| # | 子文件路径 | 章节对标 | gen 状态 | 关键内容 |
|---|-----------|---------|---------|---------|
| 01 | `critique-interaction/01-executive-summary.md` | §执行摘要 | gen-{N} | 分析视角声明、审查立场、审问对象概要、Karpathy四原则审查表 |
| 02 | `critique-interaction/02-challenged-assumptions.md` | §被挑战的假设 | gen-{N} | 逐项反对意见（交互决策）、设计偏见检测结果 |
| 03 | `critique-interaction/03-edge-cases.md` | §边界情况 | gen-{N} | 边界情况发现、烂代码检测（布局模板化/数据虚假化/状态缺失/密度失衡）、YAGNI审问结果 |
| 04 | `critique-interaction/04-recommendations.md` | §建议 | gen-{N} | 🔴升级裁决的争议、⚠️向协调器汇报 |

#### Phase 8b: critique-visual/（视觉审问）

| # | 子文件路径 | 章节对标 | gen 状态 | 关键内容 |
|---|-----------|---------|---------|---------|
| 01 | `critique-visual/01-executive-summary.md` | §执行摘要 | gen-{N} | 分析视角声明、审查立场、审问对象概要、WCAG基线 |
| 02 | `critique-visual/02-accessibility-audit.md` | §可访问性审计 | gen-{N} | WCAG 2.1 AA 逐项审核（对比度/触摸目标/键盘导航/屏幕阅读器）、可访问性相关反对意见 |
| 03 | `critique-visual/03-visual-consistency.md` | §视觉一致性 | gen-{N} | 设计令牌一致性检查、组件库一致性、烂代码检测（字体/配色/文案痕迹/密度失衡）、Karpathy四原则审查表 |
| 04 | `critique-visual/04-recommendations.md` | §建议 | gen-{N} | 🔴升级裁决的争议、⚠️向协调器汇报 |

### INDEX 文件 mini-模板

---

#### crit-ixd-INDEX.md（Phase 6b 子索引）

```
> 🚨 Agent: critic | gen: gen-{N} | Status: ✅
> 🚨 导航指令：本 INDEX 列出 critique-interaction/ 下全部子文件。读取者必须 Read 实际子文件，不可仅读此 INDEX 摘要。

# 交互审问索引 (Critique Interaction INDEX)

| # | 子文件 | 章节对标 | 状态 | gen |
|---|--------|---------|------|-----|
| 01 | 01-executive-summary.md | §执行摘要 | ✅ | gen-{N} |
| 02 | 02-challenged-assumptions.md | §被挑战的假设 | ✅ | gen-{N} |
| 03 | 03-edge-cases.md | §边界情况 | ✅ | gen-{N} |
| 04 | 04-recommendations.md | §建议 | ✅ | gen-{N} |

> 🚨 文件底提醒：此 INDEX 仅为导航。所有审问内容在子文件中。读取者必须 Read 实际子文件。
```

---

#### crit-ui-INDEX.md（Phase 8b 子索引）

```
> 🚨 Agent: critic | gen: gen-{N} | Status: ✅
> 🚨 导航指令：本 INDEX 列出 critique-visual/ 下全部子文件。读取者必须 Read 实际子文件，不可仅读此 INDEX 摘要。

# 视觉审问索引 (Critique Visual INDEX)

| # | 子文件 | 章节对标 | 状态 | gen |
|---|--------|---------|------|-----|
| 01 | 01-executive-summary.md | §执行摘要 | ✅ | gen-{N} |
| 02 | 02-accessibility-audit.md | §可访问性审计 | ✅ | gen-{N} |
| 03 | 03-visual-consistency.md | §视觉一致性 | ✅ | gen-{N} |
| 04 | 04-recommendations.md | §建议 | ✅ | gen-{N} |
...
#### critique-interaction/01-executive-summary.md
...
🚨 Guardrails:
- 禁止跳过分析视角声明——必须明确审查立场
- 禁止空洞摘要——必须列出具体的反对意见数量和状态分布
- Karpathy 四原则审查表必须逐条评估并给出具体问题（不能只写"通过"）
...
#### critique-interaction/02-challenged-assumptions.md
...
🚨 Guardrails:
- 禁止为反对而反对——每个反对意见必须有依据
- 禁止无替代方案的批评——每个反对意见必须含 ≥1 个替代方案
- 禁止批量挑战——每个反对意见独立成块
- 禁止笼统批评——"这个流程感觉不对"不合格，必须引用具体元素和违反的原则
- 禁止放过任何 AI 默认烂代码模式（三列等宽卡片/假数据）

#### 质量自检
- [ ] 每个反对意见是否包含对象/问题/影响/依据/替代方案/状态？
- [ ] 每个反对意见是否有 ≥1 个替代方案？
- [ ] 是否逐项检测 5 种 UX 设计偏见？
- [ ] 是否检测了 AI 默认烂代码模式？
```
...
#### critique-interaction/03-edge-cases.md
...
## 边界情况发现
| 边界 | 涉及决策点 | 当前设计覆盖？ | 反对意见引用 |
|------|-----------|-------------|------------|

## 烂代码（Slop）检测结果
| 检测项 | 具体检查 | 是否发现 | 严重程度 |
|--------|---------|---------|---------|
| 布局模板化 | 是否三列等宽卡片？卡片宽高比是否全部相同？ | ✅/❌ | critical/major/minor/suggestion |
| 数据虚假化 | 是否有 99.99% 等假数字？是否 John Doe 等假名？ | ✅/❌ | ... |
| 状态缺失 | 空/加载/错误状态是否被跳过？ | ✅/❌ | ... |
| 密度失衡 | VISUAL_DENSITY>7 却没有紧凑设计？<3 却要素过多？ | ✅/❌ | ... |

## YAGNI 审问结果
[为低概率场景准备的过度设计 → 标注复杂度通胀点]

🚨 Guardrails:
- 禁止跳过边界情况——每个关键决策点必须考虑边界
- 烂代码检测必须逐项执行（4 项全覆盖）
- 禁止发现烂代码但不标注严重程度
- YAGNI 结果必须具体（不能只写"存在过度设计"——必须指出哪个设计为哪个低概率场景准备了复杂度）
...
#### critique-interaction/04-recommendations.md
...
🚨 Guardrails:
- 禁止"建议"为空——每个升级争议必须有双边立场
- 禁止升级原因笼统——必须写清"为什么在 critic 层面无法达成一致"
- 向协调器汇报中必须明确列出需要裁决的具体争议点

#### 质量自检
- [ ] 🔴升级争议是否列出双边立场？
- [ ] 升级原因是否具体？
- [ ] ⚠️向协调器汇报是否包含需要裁决的具体争议点？
```

---

#### critique-visual/01-executive-summary.md
...
## 审问对象
visual-design/ 下的关键决策点：[列出]

## WCAG 基线
[本次审问使用的 WCAG 版本和等级目标]
...
#### critique-visual/02-accessibility-audit.md
...
🚨 Guardrails:
- 🔴 禁止不标注 WCAG 合规等级——每个检查项必须标注 A/AA/AAA 和合规状态
- 禁止跳过任何 WCAG 检查项（7 项必须全覆盖）
- 禁止只标注"通过/不通过"不给具体值——对比度和尺寸必须给具体数值
...
#### critique-visual/03-visual-consistency.md
...
| 检测项 | 具体检查 | 是否发现 | 严重程度 |
|--------|---------|---------|---------|
| 字体默认化 | 是否用了 Inter/Roboto？是否有字体选择理由？ | ✅/❌ | critical/major/minor/suggestion |
| 配色 AI 化 | 是否有紫色渐变？强调色饱和度是否 >80%？ | ✅/❌ | ... |
| 布局模板化 | 卡片宽高比是否全部相同？是否居中 Hero？ | ✅/❌ | ... |
| 文案痕迹化 | 是否有 em-dash？是否有 "Seamless"/"Elevate" 类 AI 营销语？ | ✅/❌ | ... |
| 密度失衡 | VISUAL_DENSITY>7 却没有紧凑设计？<3 却要素过多？ | ✅/❌ | ... |

## 设计旋钮一致性检查
| 旋钮 | 设定值 | 实际设计 | 一致性 |
|------|--------|---------|--------|
| DESIGN_VARIANCE | {N} | [表现] | ✅/❌ |
| VISUAL_DENSITY | {N} | [表现] | ✅/❌ |
| COLOR_BOLDNESS | {N} | [表现] | ✅/❌ |
...
🚨 Guardrails:
- 🔴 禁止不引用设计令牌——视觉一致性每项检查必须引用具体令牌名
- 🔴 禁止不检查设计旋钮一致性（如 VARIANCE=8 但布局完全对称 → 必须报告）
- 禁止放过任何 AI 默认烂代码模式（Inter字体/紫色渐变/三列等宽卡片/假数据/em-dash）
- 5 项烂代码检测必须逐项执行
...
#### critique-visual/04-recommendations.md
...
🚨 Guardrails:
- 禁止"建议"为空（同 Phase 6b 04-recommendations.md）
- 禁止升级原因笼统
