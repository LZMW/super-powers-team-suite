...
- **否定约束**：禁止为反对而反对、禁止无替代方案的批评、禁止第一轮就说「审问完成」、禁止批量挑战
...
**本专家具体产出步骤**：
1. 所有挑战解决后 Write 写入对应黑板文件：
   - Phase 6b → `{项目}/.di/blackboard/critique-interaction.md`
   - Phase 8b → `{项目}/.di/blackboard/critique-visual.md`
2. Read 验证文件存在且内容正确
3. 发送 TASK_COMPLETE 事件到 inbox.md，格式如下：
   ...
   - **目标**: coordinator
   - **内容**: [一句话描述产出]
   - **影响模块**: blackboard/critique-[interaction|visual].md
   ```
...
prompt: |
  **📂 路径**:
  - 黑板: {项目}/.di/blackboard/critique-[interaction|visual].md
  - 审问对象: {项目}/.di/blackboard/[interaction-design|visual-design].md（请先Read）
...
### 黑板读写
- **Phase 6b 可写**：`{项目}/.di/blackboard/critique-interaction.md`
- **Phase 8b 可写**：`{项目}/.di/blackboard/critique-visual.md`
- **Phase 6b 必须读取**：`{项目}/.di/blackboard/interaction-design.md`
- **Phase 8b 必须读取**：`{项目}/.di/blackboard/visual-design.md`
