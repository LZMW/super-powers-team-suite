...
|---|-----------|---------|---------|---------|
| 01 | `architecture-analysis/01-codebase-overview.md` | §代码库概览 | gen-{N} | 分析视角声明、技术栈指纹（四维度）、项目类型判定、项目概览 |
| 02 | `architecture-analysis/02-architecture-patterns.md` | §架构模式清单 | gen-{N} | 架构风格、分层设计、组件关系图(Mermaid)、数据流、调用链分析、设计模式、关键设计决策 |
| 03 | `architecture-analysis/03-module-boundaries.md` | §模块边界 | gen-{N} | 模块职责、依赖关系、接口定义、关键度评估 |
| 04 | `architecture-analysis/04-tech-stack-assessment.md` | §技术栈评估 | gen-{N} | 技术栈详表（技术/版本/用途/选型评价）、🔬交叉印证（如上游存在）、⚠️向协调器汇报 |
...
```
> 🚨 Agent: analyst | gen: gen-{N} | Status: ✅
...
- 禁止评价代码风格
- 禁止主观评判（如"这个设计好/坏"）
- 禁止把推断写成事实（必须标注确定性：确认的事实 vs 合理推断 vs 待验证假设）
- 禁止空洞评价（如"该设计具有高内聚"——必须写"从{品味向量}视角评估，该设计的高内聚体现在…"）
- 项目类型判定必须有依据（列出具体判定特征，不可仅凭直觉）
- 四维度指纹扫描必须全部覆盖（语言/运行时、框架/基座、数据/中间件、构建/部署）
...
🚨 Guardrails:
- 禁止把推断写成事实（推断理由标注确定性等级）
- 禁止空洞评价（每个评估必须具体——"决策X值得追问为什么不用Y"而非"可以讨论架构"）
- 禁止不标注调用链文件:行号证据
- 每条调用链必须追踪到数据层或外部调用，不遗漏分支和异常路径
- 禁止跳过关键业务流程（至少追踪 2-3 条核心流程）
- 禁止评价代码风格

#### 质量自检
- [ ] 架构风格是否准确判定？
- [ ] 组件关系图是否使用 Mermaid 或 ASCII art？
- [ ] 每条调用链是否追踪到数据层或外部调用？
- [ ] 调用链是否覆盖分支和异常路径？
- [ ] 设计模式是否标注实现位置（文件:行号）？
- [ ] 关键设计决策是否标注确定性？
- [ ] 每个关键发现是否标注对 interrogator 的拷问价值？
```
...
- 禁止不写选型评价（每个技术必须有评价）
- 禁止省略技术用途说明
- 禁止把交叉印证中的矛盾发现悄悄忽略——必须标注并分析根因
- 交叉印证中 design-miner 的贡献必须标注来源

#### 质量自检
- [ ] 技术栈是否包含版本号和用途说明？
- [ ] 选型评价是否有依据？
- [ ] 交叉印证（如上游存在）：是否逐条对照？矛盾是否有根因分析？
- [ ] 是否标注了"确认的事实 vs 合理推断 vs 待验证假设"？
- [ ] ⚠️向协调器汇报中是否列出了需要特别关注的发现？
```
