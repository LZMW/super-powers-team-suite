...
### DG — Dev-Genius（开发天才）v3.3

**读取 DI 设计规格，按八道门控完成开发**。7 位专家串行执行，每道门不过不许往下走。纯数字 Gate 编号=严格串行，无并行 Gate。

```
Gate 0: F(开发前源码扫描)
Gate 1: Planner(任务分解)
Gate 2: Architect(三级响应：完整架构/影响评估/架构签批)
Gate 3: L(文档校验——Planner↔Architect 一致性)
Gate 4: Developer(TDD: RED→GREEN→REFACTOR)
Gate 5: QA Tester(验收验证，证据驱动)
Gate 6: Analyst(Karpathy四原则 + OWASP审查)
Gate 7: F(开发后重扫描 + 差异报告)
Gate 8: Coordinator(汇总交付)
```

F(源码状态分析师) 是唯一运行两次的专家——Gate 0 拍基准快照，Gate 7 拍结果快照，diff = 净产出。
...
维护者：Mr.Chen · 版本 v4.1 · 更新日期 2026-05-30
