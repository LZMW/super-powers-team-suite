...
| **DG** (开发实现) | `Step 0-9` + `Gate 1-5` + `.5` 小数 | **Gate 1-6** | ✅ 三团队唯一 |

**关键规则**：三个前缀互不重叠。对话中提到 `Stage` 必指 DM，`Phase` 必指 DI，`Gate` 必指 DG。
...
```
DM (Stage 1-7) → DI (Phase 0-12) → DG (Gate 1-6)
```
