...
**GENESIS.md 追加**：每次回退/重做/矛盾解决追加一条记录（格式见黑板结构）。
...
- 每轨道完成后立即将黑板模块内容追加到对应的 `output/` 文档
- 追加格式：时间戳标题 + 轨道标签 + 完整内容，追加在已有内容之后
- 部分流程（仅架构/仅UX/仅方法论）结束时同样触发——不遗漏任何分析结果
- 禁止等「全部做完再一起写」——那是旧模式，部分流程的产出会丢失
...
| 🔴 专项聚焦分析 | 用户指定具体主题（"缓存策略"/"安全审计"/"错误处理"） | 动态选择相关专家 | 按需 | 相关黑板模块 | `05-{主题}.md`（自定义命名） |
...
**命名规则**：标准文档 01-04 前缀固定。自定义文档使用 `05-{kebab-case主题}.md`，如 `05-缓存策略分析.md`、`05-安全性分析.md`。
...
| [05-缓存策略分析.md](./05-缓存策略分析.md) | Redis缓存策略、失效模式 | 2026-05-22 | 专项：架构 D |
| ... | ... | ... | ... |
