...
> 🔴 v5.2 核心变化：产出文档从「完整流程最后一次性生成」改为「每轨道完成后立即追加」。部分流程也能留下记录。

**输出模式**：累积追加。每次分析会话在已有文档末尾追加新章节，以时间戳和轨道标签区分。从不覆盖已有内容。
...
| **任何会话结束** | `00-综合报告.md` | 本次运行的轨道摘要 + 品味向量 + 视角局限性声明 |
...
- **品味向量**: [本次使用的基向量]
- **产出摘要**: [一页概览]
- **视角局限性**: "给定我们选择的{偏好}视角，以下发现可能不适用于关注{其他维度}的读者"
```
