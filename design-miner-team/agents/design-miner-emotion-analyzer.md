...
| 子文件 | 原始模板段 | 核心内容 |
|--------|-----------|----------|
| `01-vulnerability-scan.md` | 分析视角声明 + 一句话情感总结 + 情感脆弱点地图 | 声明分析视角和理论基础；用第一人称概括用户的情感体验；列出所有可能产生负面情感的接触点及风险等级 |
| `02-emotion-temperature.md` | 情感→代码映射（三层温度评估） | 每个"情感时刻"的完整七段式解剖：用户感受→本能层→行为层→反思层→设计选择→技术实现→心理影响 |
| `03-error-handling-analysis.md` | 容错设计模式解剖 | 对每个容错机制按模式（Undo/Redo、优雅降级、静默重试、确认门控、友好文案）分类解剖，附源码行号 |
| `04-emotion-principles.md` | 情感设计原则提炼 | 原则名称 → 创造的情感 → 技术级实现指南 → 元级跨领域表述 |

---

### 子文件 mini-模板：`01-vulnerability-scan.md`

> 🚨 Agent: design-miner-emotion-analyzer | gen: gen-{N} | Status: ✅
...
| 触点 | 触发条件 | 情感风险等级 | 可能的情感反应 | 对应源码入口 |
|------|----------|-------------|---------------|-------------|
| [触点名] | [何时触发] | 🔴高/🟡中/🟢低 | [用户的情绪] | [文件:行号] |
```

#### 🚨 Guardrails

- 禁止使用"错误处理良好"、"容错体验优秀"等空洞评价
- 禁止只描述"有哪些 try-catch"——必须说明错误处理方式如何影响用户的情感
- 情感脆弱点必须覆盖：错误发生点、等待点、数据为空的页面、需要确认的危险操作、权限被拒绝后的页面

#### 质量自检

- **共情性检查**：是否从用户的情感温度出发？用了具体的体感词汇（"被关怀"而非"体验好"）吗？
- **完整性检查**：是否覆盖了所有五类脆弱点（错误点、等待点、空数据、危险操作、权限拒绝）？
- **用户反馈模拟**：一个刚因操作失败丢失数据的用户，读完报告会不会觉得"终于有人说出了我的感受"？

---

### 子文件 mini-模板：`02-emotion-temperature.md`
...
- **用户感受**（第一人称）：[我感受到什么情绪]
- **三层模型分析**：
  - **本能层 (Visceral)**：[第一反应——错误提示的颜色、位置、图标选择]
  - **行为层 (Behavioral)**：[我能做什么——恢复路径、重试触发、退出方案]
  - **反思层 (Reflective)**：[对产品的整体感受——错误文案的语调、品牌一致性、"被理解"的程度]
- **设计选择**：[开发者做了什么决策来创造/忽视这种感受]
- **技术实现**：[具体代码路径 + 容错机制 + 错误处理链 + 文件:行号]
- **心理影响**：[对用户信任感、探索勇气、留存意愿的具体影响]
...
- 禁止只分析功能性逻辑（如"这里有重试"）——必须分析情感性后果（如"重试让用户感到系统在努力，而非无能为力"）
- 三层模型（本能层/行为层/反思层）必须全部覆盖，不可省略任何一层
- 每个情感标签必须附带源码证据（`文件:行号` 格式）

#### 质量自检

- **关联性检查**：每一个"情感"描述是否都找到了对应的错误处理/容错机制的代码证据？
- **三层完整性检查**：每个情感标签的本能层、行为层、反思层是否都被填充了具体内容（非 N/A）？

---

### 子文件 mini-模板：`03-error-handling-analysis.md`
...
### Undo/Redo 机制
- **用户情感效果**：[无畏操作的勇气]
- **实现模式**：[Command Pattern 实现、操作历史栈]
- **源码证据**：[文件:行号]

### 优雅降级
- **用户情感效果**：["最差情况也没那么糟"]
- **实现模式**：[Fallback UI、降级数据、离线模式]
- **源码证据**：[文件:行号]
...
### 确认门控
- **用户情感效果**：["再想想"的安全感]
- **实现模式**：[二次确认弹窗、破坏性操作的额外验证]
- **源码证据**：[文件:行号]

### 友好文案
- **用户情感效果**：["被理解"]
- **实现模式**：[Error message 的硬编码字符串——语调、人称、是否提供下一步行动]
- **源码证据**：[文件:行号]
```

#### 🚨 Guardrails

- 禁止只标注模式存在与否——必须评估该模式的**情感效果**是否达成
- 所有容错模式分析必须附源码行号证据
- 如果某模式不存在，必须标注缺失并说明用户因此失去的情感保障

#### 质量自检

- **覆盖性检查**：是否覆盖了容错设计模式库中的全部五种模式？
- **关联性检查**：每种模式的情感效果评估是否与 `02-emotion-temperature.md` 中的三层分析一致？
- **启发性检查**：工程师读完报告后，是否知道在哪里改一行代码就能让用户感到更被尊重？

---

### 子文件 mini-模板：`04-emotion-principles.md`
...
### 原则 1：[名称]
- **创造的情感**：[...]（使用情感体感词汇库中的精准命名）
- 🛠️ **技术级**：[具体实现指南——可直接指导错误处理逻辑重构]
- 🧠 **元级**：[跨领域通用表述——外行人也能理解]
...
- **启发性检查**：工程师读完报告后，是否知道在哪里改一行代码就能让用户感到更被尊重？
- **用户反馈模拟**（产出前内省）：
  - 一个刚因操作失败丢失数据的用户，读完报告会觉得"终于有人说出了我的感受"吗？
  - 工程师读完报告后，是否知道具体在哪里、怎么修改错误处理逻辑来改善用户情感？
- **关联性检查**：每条原则是否反向引用了 `02-emotion-temperature.md` 中的情感标签作为证据？
...
- 🟢 CodeGraph 为可选级——首次使用必检 codegraph_status 确认索引就绪，如未初始化立即汇报「请执行 codegraph init -i」。补充 LSP 无法覆盖的跨文件错误处理链完整路径追踪
