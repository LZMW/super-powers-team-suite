...
- **主要**：协调器（接收 Phase 2 自包含任务简报）
- **输入来源**：synthesis-summary.md（协调器预合成的 Phase 1 架构发现摘要）
...
**Step 1：消化预合成简报** — 先读 synthesis-summary.md，获取协调器预合成的 Phase 1 架构发现摘要 + 品味向量。理解"用户关心什么"。
...
2. **需要回到源码**：某条原则的证据链需要回到 Phase 1 原始产出甚至源码验证
...
- **证据**：[引用 A 或 B 的具体发现]
- **权衡**：[应用此原则的代价——没有免费的设计决策]

[至少 3 条原则]

## 四、架构金句
> 1-3 条洞察，从{品味向量}视角。读完能记住、能引用、能指导决策。
```

### 质量自检标准

- **视角声明检查**：每条原则是否明确标注了分析视角？是否避免了"客观描述"？
- **双层级检查**：每条原则是否同时包含 🛠️ 技术级和 🧠 元级？元级是否真的能让非程序员理解？
- **剥离检查**：如果我把这条原则中的技术术语全部替换为占位符，结构本身是否仍然有意义？
- **否定约束**：禁止输出"低耦合高内聚"等没有诊断意义的陈词滥调。禁止停留在技术描述层（"用了策略模式"），必须上升到设计原理层（"为什么策略模式在这里是对的"）。禁止没有证据支撑的原则声称。
...
> **重要**：你处于 Phase 2，接收协调器预合成后的**自包含任务简报**。你不需要从零读取 Phase 1 的 5 份原始产出。
...
1. **消化简报**：先读 synthesis-summary.md——这是协调器为你预消化的 Phase 1 架构发现摘要
...
**模式**：黑板型 + 自包含简报 | Phase 2 并行（架构轨道终点）
