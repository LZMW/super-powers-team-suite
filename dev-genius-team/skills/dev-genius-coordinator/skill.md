...
| **专家代号** | F:codebase-analyst(Gate 0+7) → Planner→Architect（三级响应）→Doc-Validator(Gate 3)→Developer↔QA Tester→Analyst |
...
| **铁律** | F 双阶段(Gate 0+7)、Architect 必经（三级响应）、QA Tester 必经（证据驱动）、Developer 遇架构冲突→停止→回退 |
...
开发流程必须经过八道质量门控，**每道门控未通过时禁止进入下一阶段**。Gate 2 (Architecture Gate) 和 Gate 5 (Verify Gate) 是必经门控——协调器不可跳过。
...
| Gate 7 | codebase-state/codebase-INDEX.md | 存在 + 含更新的 01-04 + 06-post-dev-diff.md + DI 覆盖度验证 | 退回 F |
...
|----------|----------|----------|----------------|---------------|----------|
| 新项目全流程 | F(Gate 0)→Planner→Architect→Doc-Validator→Developer→QA→Analyst→F(Gate 7) | 全部 7 道门控 + Gate 3 | 完整架构 | 每任务验收验证 | 全部8文件夹 |
...
| 多模块并行开发 | 并行 Developer 实例 + 统一 Architect/Doc-Validator/QA/Analyst | 全部 6 道门控 + Gate 3 | 完整架构 | 每模块验收验证 | 黑板状态同步 |
...
| doc-validator (L) | Gate 3 | doc-state/ | doc-INDEX.md | architecture/ + task-queue/ + DI phases/ |
...
| analyst | Gate 6+7 | review-report/ | review-INDEX.md | code-state/ + architecture/ + test-report/ + codebase-state/05 |
...
- Developer 在 Gate 4 发现实现与 architecture.md 冲突 → 停止 → 回退到 Gate 2
- 🔴 协调器追加 GENESIS.md 回退记录
- Architect 重新评估：
  - 如 architecture.md 原设计需调整 → 旧章节标记 `> ⚠️ [STALE — gen-N]` → 追加新结论
  - 如原设计正确 → 不标记 STALE，仅追加签批确认
- 🔴 Architect 完成后 → 协调器重新触发 Developer 回到 Gate 4，继续实现
...
**Gate 3 检查**：Read doc-state/doc-INDEX.md → 存在 + 含架构文档与 DI 规格一致性校验 + 接口契约验证通过 + ADR 追溯完整？→ 通过则继续
...
**Gate 4 检查**: Read code-state/code-INDEX.md → 有测试命令和通过结果？→ 通过则继续
...
**Gate 5 检查**: Read test-report/test-INDEX.md → 验收标准全部通过？→ 通过则继续。有 Bug？→ 触发 Dev↔QA 闭环
...
**Gate 6 检查**: Read review-report/review-INDEX.md → 无 Critical？→ 通过
...
**Gate 7 检查**: Read codebase-state/codebase-INDEX.md → 存在 + 含更新的 01-04 + 06-post-dev-diff.md + DI 覆盖度验证？→ 通过则继续
...
- ✅ Gate 3 (Doc-Validate Gate): doc-state/doc-INDEX.md — 架构文档与 DI 规格一致性校验通过，接口契约验证通过，ADR 追溯完整
...
| F Gate 7 差异对比遗漏 | codebase-analyst 开发后重扫描未生成 06-post-dev-diff.md | Gate 7 检查验证 06-post-dev-diff.md 存在；缺失 → 退回 F 重新扫描 |
