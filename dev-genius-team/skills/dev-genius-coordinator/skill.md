...
description: Dev-Genius (开发天才) team coordinator skill. Reads upstream design specs from design-interrogator-team, communicates with users, and coordinates 6 expert agents (dev-genius-codebase-analyst, dev-genius-planner, dev-genius-architect, dev-genius-developer, dev-genius-qa-tester, dev-genius-analyst) using Blackboard pattern with Event Bus for state synchronization and mandatory seven-gate pipeline (Codebase Gate → Plan Gate → Architecture Gate → TDD Gate → Verify Gate → Review Gate → Codebase Re-scan Gate → Finish Gate). Architect and QA Tester are required gates. Use when user needs full-cycle software development, code implementation with TDD enforcement, autonomous development loops, or any software engineering tasks requiring multi-expert collaboration.
---
...
| **专家代号** | F:codebase-analyst(Gate 0+6) → Planner→Architect（三级响应）→Developer↔QA Tester→Analyst |
| **流程顺序** | Gate 0(F:开发前源码扫描) → Gate 1(Plan) → Gate 2(Arch) → Gate 3(TDD) → Gate 4(Verify) → Dev↔QA闭环 → Gate 5(Review) → Gate 6(F:开发后重扫描) → Gate 7(Finish) |
| **关键文件** | MASTER-INDEX.md → codebase-state/ → task-queue/ → architecture/ → code-state/ → test-report/ → review-report/ |
...
| Gate 7 | 全部 6 个文件夹 | 全部任务完成 + 全部测试通过 + 0 Critical + DI 覆盖度验证通过 | 不可交付 |
...
|----------|----------|----------|----------------|---------------|----------|
| 新项目全流程 | F(Gate 0)→Planner→Architect→Developer→QA→Analyst→F(Gate 6) | 全部 7 道门控 | 完整架构 | 每任务验收验证 | 全部7文件夹 |
| 单功能开发 | Planner(轻量)→Architect→Developer→QA→Analyst | Gate 1+2+3+4+5 | 影响评估 | 每任务验收验证 | code-state, test-report, review-report |
| Bug 修复 | Planner(单任务)→Architect→Developer↔QA | Gate 1+2+3+4 | 架构签批 | 修复验证+回归测试 | code-state, test-report |
...
└── blackboard/
    ...
    ├── task-queue/         # Planner（3子文件+task-INDEX.md）
    ├── architecture/       # Architect（6子文件+arch-INDEX.md）
...
| planner | Gate 1 | task-queue/ | task-INDEX.md | DI phases/ + codebase-state/04+05 |
| architect | Gate 2 | architecture/ | arch-INDEX.md | task-queue/ + DI phases/ + codebase-state/02+03 |
...
创建 `.dev-genius/` 目录结构（含 `blackboard/` 子目录下的全部 7 个文件夹 + `GENESIS.md` + `MASTER-INDEX.md`）。GENESIS.md 初始化为空文件（首行 `# 世代日志`），后续事件追加。
...
├── task-queue/ (含 task-INDEX.md + 3 子文件)
├── architecture/ (含 arch-INDEX.md + 6 子文件)
...
- [ ] ✅ 包含所有 6 个专家名称（F: codebase-analyst 含在内）
...
- [ ] ✅ 黑板数据结构已定义（7文件夹 + MASTER-INDEX.md + inbox.md）
- [ ] ✅ 黑板读写权限矩阵已配置（7行权限表）
