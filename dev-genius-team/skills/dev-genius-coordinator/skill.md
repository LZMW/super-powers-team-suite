...
🔴 MASTER-INDEX 闭环更新
    ↓
🔴 DI 覆盖率汇报
    Read post-dev-diff/ 最新子文件 → 提取覆盖率 + 未实现项
...
> ⚠️ **关键步骤**：MASTER-INDEX 更新后，必须汇报 DI 覆盖度并与用户确认未实现项！

**流程**：
```
1. Read post-dev-diff/post-dev-diff-INDEX.md → 定位最新 gen 子文件
2. Read 最新子文件 → 提取 DI 覆盖度数据和未实现项清单
3. 计算覆盖率：已实现 N / DI 要求总数 M = X%
4. 若覆盖率 < 100%:
   a. 逐项列出未实现项 + 未实现原因（从 post-dev-diff 的 DI 覆盖度验证表提取）
...
| 未实现项 | 原因 |
|---------|------|
| [要求 1] | [原因——如: 非本轮范围 / arch 共识推后 / 需独立诊断] |
| [要求 2] | [原因] |
