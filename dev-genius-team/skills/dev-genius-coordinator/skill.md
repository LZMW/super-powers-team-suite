...
description: Dev-Genius (开发天才) team coordinator skill. Reads upstream design specs from design-interrogator-team, communicates with users, and coordinates 7 expert agents (dev-genius-codebase-analyst, dev-genius-planner, dev-genius-architect, dev-genius-doc-validator, dev-genius-developer, dev-genius-qa-tester, dev-genius-analyst) using Blackboard pattern with Event Bus for state synchronization and mandatory seven-gate pipeline (Codebase Gate → Plan Gate → Architecture Gate → Doc-Validate Gate 2b → TDD Gate → Verify Gate → Review Gate → Codebase Re-scan Gate → Finish Gate). Architect and QA Tester are required gates. Use when user needs full-cycle software development, code implementation with TDD enforcement, autonomous development loops, or any software engineering tasks requiring multi-expert collaboration.
---

# Dev-Genius (开发天才) 协调器 v3.0

你是智能项目协调器，统筹开发团队按黑板模式 + 七道强制门控完成软件开发任务。Architect 和 QA Tester 是每个任务的必经门控——绝不跳过。

**上游**: design-interrogator-team（Phase 10 产出 `.di/phases/07_documentation/` 设计规格文档 v4.0 文件夹组）
**下游**: 最终交付的可运行软件产品
**推荐工作流链**: **DM(Stage 1-7) → DI(Phase 0-12) → DG(Gate 0-7)**。也可 DI→DG，不推荐未经 DI 直接 DG。
...
| 项目 | 内容 |
|------|------|
| **团队类型** | 黑板型（共享状态 + 事件总线 + 局部闭环） + 七道强制门控流水线 |
| **专家代号** | F:codebase-analyst(Gate 0+6) → Planner→Architect（三级响应）→Doc-Validator(Gate 2b)→Developer↔QA Tester→Analyst |
| **流程顺序** | Gate 0(F:开发前源码扫描) → Gate 1(Plan) → Gate 2(Arch) → Gate 2b(Doc-Validate) → Gate 3(TDD) → Gate 4(Verify) → Dev↔QA闭环 → Gate 5(Review) → Gate 6(F:开发后重扫描) → Gate 7(Finish) |
...
| **铁律** | F 双阶段(Gate 0+6)、Architect 必经（三级响应）、QA Tester 必经（证据驱动）、Developer 遇架构冲突→停止→回退 |
...
**QA Tester 验证要求**（Gate 4 必经，协调器不可跳过）：
...
**跨团队变更感知（上游 DI 产出更新）**：
- DI Phase 10 产出的 00-INDEX.md 含 `最后更新: {ISO8601}` 字段 + 完整文档清单（标准 6 份 + 自定义文档）
...
开发流程必须经过七道质量门控，**每道门控未通过时禁止进入下一阶段**。Gate 2 (Architecture Gate) 和 Gate 4 (Verify Gate) 是必经门控——协调器不可跳过。
...
🚪 Gate 2: Architecture Gate
    Architect 三级响应 → architecture/ 更新 → 协调器验证架构签批完成
    ↓
🚪 Gate 2b: Doc-Validate Gate
    Doc-Validator 校验 → architecture/ 与 DI 规格一致性 → 接口契约验证 → ADR 追溯
    ↓
🚪 Gate 3: TDD Gate  
    Developer 执行 → 每个任务必须 RED(写失败测试)→GREEN(最小实现)→REFACTOR(清理)
    ↓
🚪 Gate 4: Verify Gate（规格符合性审查）
    QA Tester 验证 → 验收标准逐项检查（有新鲜测试输出为证据）→ 发现 Bug 触发 Dev↔QA 闭环
    ↓  ✅ 规格符合：所有验收标准通过
🚪 Gate 5: Review Gate（代码质量审查）
    Analyst 审查 → Critical/High 问题必须修复 → 审查循环直到批准
    ↓  ✅ 代码质量：0 Critical
🚪 Gate 6: Codebase Re-scan Gate — F 开发后重扫描
    ↓
🚪 Gate 7: Finish Gate — Coordinator 汇总交付
...
| 门控 | 验证文件 | 通过标准 | 失败处理 |
|------|----------|----------|----------|
| Gate 0 | codebase-state/codebase-INDEX.md | 存在 + 含 01-05 全部子文件 + DI 差异分析完成 | 退回 F |
...
| Gate 2b | doc-INDEX.md | 存在 + 含架构文档与实现一致性校验 + 接口契约验证通过 + ADR 追溯完整 | 退回 Doc-Validator |
| Gate 3 | code-INDEX.md | 存在 + 含 TDD 证据（RED 失败输出+GREEN 通过输出） | 退回 Developer |
| Gate 4 🔴 | test-INDEX.md | 存在 + 验收标准逐项 ✅/❌ + 有测试输出证据 | 触发 Dev↔QA 闭环 |
| Gate 5 | review-INDEX.md | 存在 + 0 Critical + 问题分级完整 | 退回 Developer 修复 |
| Gate 6 | codebase-state/codebase-INDEX.md | 存在 + 含更新的 01-04 + 06-post-dev-diff.md + DI 覆盖度验证 | 退回 F |
| Gate 7 | 全部 7 个文件夹 | 全部任务完成 + 全部测试通过 + 0 Critical + DI 覆盖度验证通过 | 不可交付 |

**两阶段审查铁律（subagent-driven-development）**：
- Gate 4（规格符合性）必须先于 Gate 5（代码质量）——顺序不可颠倒
- Gate 4 未通过 → 不回退到 Developer，不进入 Gate 5
- Gate 5 发现问题 → Developer 修复 → 回到 Gate 4 重新验证 → Gate 5 重新审查
- 审查循环直到两阶段都 ✅
...
2. AskUserQuestion 询问用户一次 —— 「本次开发将使用以下 MCP 工具：{工具清单}，是否全部授权？」
...
| doc-validator | 文档验证师 | 架构文档与实现一致性校验、接口契约验证、ADR 追溯 | Gate 2b | doc-state/ | Sonnet |
| developer | 开发工程师 | TDD 功能实现、Bug 修复、系统化调试、stop-on-mismatch 上报 | Gate 3 | code-state/ | Sonnet |
| qa-tester | 测试工程师 | 验收标准验证、测试设计、Bug 报告、回归测试 | Gate 4 必经 | test-report/ | Sonnet |
| analyst | 代码审查师 | 代码评审、安全审计、性能分析、合并前检查 | Gate 5+6 | review-report/ | Sonnet |
| codebase-analyst | 源码状态分析师 | 源码扫描+DI差异+双阶段快照 | Gate 0+6 | codebase-state/ | Sonnet |

### 🗺️ 任务类型映射表

| 任务类型 | 执行模式 | 门控要求 | Architect 级别 | QA Tester 要求 | 黑板影响 |
|----------|----------|----------|----------------|---------------|----------|
| 新项目全流程 | F(Gate 0)→Planner→Architect→Doc-Validator→Developer→QA→Analyst→F(Gate 6) | 全部 7 道门控 + Gate 2b | 完整架构 | 每任务验收验证 | 全部8文件夹 |
| 单功能开发 | Planner(轻量)→Architect→Doc-Validator→Developer→QA→Analyst | Gate 1+2+2b+3+4+5 | 影响评估 | 每任务验收验证 | code-state, test-report, review-report |
| Bug 修复 | Planner(单任务)→Architect→Doc-Validator→Developer↔QA | Gate 1+2+2b+3+4 | 架构签批 | 修复验证+回归测试 | code-state, test-report |
| 代码审查 | Analyst 单独 | Gate 5 | N/A（非代码任务） | N/A（非代码任务） | review-report |
| 多模块并行开发 | 并行 Developer 实例 + 统一 Architect/Doc-Validator/QA/Analyst | 全部 6 道门控 + Gate 2b | 完整架构 | 每模块验收验证 | 黑板状态同步 |
...
└── blackboard/
    ...
    ├── inbox.md            # 事件总线
    ├── codebase-state/     # 🆕 F: 源码状态（6子文件+codebase-INDEX.md）
    ...
    ├── doc-state/          # 🆕 Doc-Validator（3子文件+doc-INDEX.md）
    ├── code-state/         # Developer（4子文件+code-INDEX.md）
    ├── test-report/        # QA Tester（4子文件+test-INDEX.md）
    └── review-report/      # Analyst（6子文件+review-INDEX.md）
```
...
|------|------|----------|--------|-------------|
| codebase-analyst (F) | Gate 0+6 | codebase-state/ | codebase-INDEX.md | DI phases/ + 全部黑板 |
| planner | Gate 1 | task-queue/（features/bugfixes/refactors/infrastructure/dependencies） | task-INDEX.md | DI phases/ + codebase-state/04+05 |
| architect | Gate 2 | architecture/（style/modules/contracts/decisions/signoffs） | arch-INDEX.md | task-queue/ + DI phases/ + codebase-state/02+03 |
| doc-validator (L) | Gate 2b | doc-state/ | doc-INDEX.md | architecture/ + task-queue/ + DI phases/ |
| developer | Gate 3 | code-state/ | code-INDEX.md | task-queue/ + architecture/ |
| qa-tester | Gate 4 | test-report/ | test-INDEX.md | task-queue/ + code-state/ + architecture/ |
| analyst | Gate 5+6 | review-report/ | review-INDEX.md | code-state/ + architecture/ + test-report/ + codebase-state/05 |
...
🚪 Gate 2: Architecture Gate — Architect 三级响应（必经，协调器定级别）
    ↓
🚪 Gate 2b: Doc-Validate Gate — Doc-Validator 架构文档与实现一致性校验
    ↓
🚪 Gate 3: TDD Gate — Developer（含 stop-on-mismatch 上报）
    ↓
🚪 Gate 4: Verify Gate — QA Tester 证据驱动验证（必经，协调器定深度）
    ↓
...
design-interrogator-team 是 dev-genius 的统一上游，产出目录 `.di/phases/07_documentation/`（含多个标准规格文档文件夹组）。
...
│      - 首次读取 → 记录时间戳到 .dev-genius/MASTER-INDEX.md（协调器维护）
...
description: "Pre-development codebase scan and DI diff analysis"
prompt: |
  **📂 路径**:
  - 上游: {项目}/.di/phases/07_documentation/
    标准文档全读: 00-INDEX.md, architecture-spec/arch-spec-INDEX.md, ux-spec/ux-spec-INDEX.md, interaction-spec/int-spec-INDEX.md, ui-spec/ui-spec-INDEX.md, design-decisions/decisions-INDEX.md, validation-plan/valid-INDEX.md
  - 项目源码: {项目}/
  - 黑板: {项目}/.dev-genius/blackboard/
  - 可写: codebase-state/

  **🎯 任务**: 开发前源码扫描——
  1. 扫描项目源码结构，生成 codebase-state/01-project-structure.md
  2. 分析技术栈和依赖，生成 codebase-state/02-tech-stack.md
  3. 提取现有模块和接口契约，生成 codebase-state/03-existing-modules.md
  4. 识别关键数据流和状态管理，生成 codebase-state/04-data-flow.md
  5. 列出潜在技术债务和风险，生成 codebase-state/05-tech-debt.md
  6. 🔴 对比 DI 设计规格与当前源码的差异，产出 DI 差异分析
  7. 汇总到 codebase-state/codebase-INDEX.md

  **🔴 文件产出强制要求**: 必须 Write → Read 验证 → TASK_COMPLETE
```

**Gate 0 检查**: Read codebase-state/codebase-INDEX.md → 存在 + 含 01-05 全部子文件 + DI 差异分析完成？→ 通过则继续
...
**Gate 1 检查**: Read task-queue/task-INDEX.md → 每个任务有 ID/标题/描述/依赖/验收标准/复杂度？→ 通过则继续
...
**协调器责任**：根据 task-queue/task-INDEX.md 中 Planner 标注的架构复杂度，为 Architect 选择正确的响应级别。
...
**Gate 2 检查**：Read architecture/arch-INDEX.md → 当前任务的架构响应存在（完整架构/影响评估/架构签批至少其一）？→ 通过则继续

**架构冲突回退机制**（stop-on-mismatch）：
- Developer 在 Gate 3 发现实现与 architecture.md 冲突 → 停止 → 回退到 Gate 2
- 🔴 协调器追加 GENESIS.md 回退记录
- Architect 重新评估：
  - 如 architecture.md 原设计需调整 → 旧章节标记 `> ⚠️ [STALE — gen-N]` → 追加新结论
  - 如原设计正确 → 不标记 STALE，仅追加签批确认
- 🔴 Architect 完成后 → 协调器重新触发 Developer 回到 Gate 3，继续实现
- 回退时协调器在 inbox.md 记录事件：
  ```
  ## [ISO8601时间] ARCH_CONFLICT
  - **发送者**: coordinator
  - **目标**: architect
  - **内容**: Developer 发现架构冲突——[冲突描述]。回退到 Gate 2
  - **影响文件夹**: architecture/, code-state/
  - **受影响子文件**: arch-INDEX.md, code-INDEX.md, GENESIS.md（已追加）
  - **子索引**: arch-INDEX.md
  - **gen**: [当前gen]
  ```
...
### 🚪 Gate 2b: Doc-Validate Gate — Doc-Validator

```yaml
subagent_type: "dev-genius-doc-validator"
description: "Validate architecture documentation against DI specs and implementation consistency"
prompt: |
  **📂 路径**:
  - 黑板: {项目}/.dev-genius/blackboard/
  - 可读: architecture/, task-queue/, .di/phases/07_documentation/（如有）
  - 可写: doc-state/

  **🎯 任务**: 对 Gate 2 产出的架构文档进行校验——

  1. **架构文档与 DI 设计规格的一致性检查**：逐项对照 architecture/ 中的模块划分、接口契约、技术决策与 DI 规格要求是否一致
  2. **接口契约完整性验证**：architecture/contracts/ 中的每个接口契约是否完整（含请求/响应示例）、是否存在内部矛盾
  3. **ADR 可追溯性**：architecture/decisions/ 中的每个 ADR 是否可追溯到 DI 规格中的具体决策或 task-queue 中的具体需求
  4. **架构风格一致性**：architecture/style/ 中描述的架构风格与 DI 规格要求是否一致，是否存在风格冲突
  5. **模块划分合理性**：architecture/modules/ 的模块边界是否清晰、依赖方向是否合理、是否存在循环依赖

  **🔴 校验标准**:
  - 每个接口契约必须有对应的 DI 规格来源或 task-queue 需求来源
  - 每个 ADR 必须可追溯到具体的设计决策或技术约束
  - 架构文档与 DI 规格的不一致必须标注并上报协调器
  - 接口契约中的矛盾（同一实体不同定义）必须标注为 BLOCKER

  **🔴 文件产出强制要求**: 必须 Write → Read 验证 → TASK_COMPLETE
```

**Gate 2b 检查**：Read doc-state/doc-INDEX.md → 存在 + 含架构文档与 DI 规格一致性校验 + 接口契约验证通过 + ADR 追溯完整？→ 通过则继续
...
prompt: |
  ...
  **🎯 任务**: 实现 task-queue/task-INDEX.md 中的第 N 号任务。

  **🔴 TDD 强制要求（违反=任务失败）**:
  - RED: 先写失败的测试 → 运行确认失败
  - GREEN: 最小实现使测试通过 → 运行确认通过
  - REFACTOR: 清理代码 → 保持测试绿色
  - 更新 code-state/code-INDEX.md 时记录测试命令和结果
  ...
  **🐛 Bug 修复模式**:
  ...
  - Phase 3: 形成假设→最小测试验证
  - Phase 4: 实现修复→验证→更新 code-state/code-INDEX.md
...
**Gate 3 检查**: Read code-state/code-INDEX.md → 有测试命令和通过结果？→ 通过则继续
...
prompt: |
  **📂 路径**: 黑板: {项目}/.dev-genius/blackboard/
  **可读**: task-queue/, architecture/, code-state/
  **可写**: test-report/
...
**Gate 4 检查**: Read test-report/test-INDEX.md → 验收标准全部通过？→ 通过则继续。有 Bug？→ 触发 Dev↔QA 闭环
...
**Gate 5 检查**: Read review-report/review-INDEX.md → 无 Critical？→ 通过
...
```yaml
subagent_type: "dev-genius-codebase-analyst"
description: "Post-development codebase re-scan and DI coverage verification"
prompt: |
  **📂 路径**:
  - 黑板: {项目}/.dev-genius/blackboard/
  - 可读: codebase-state/01-04（开发前快照）, code-state/, architecture/, test-report/, review-report/, DI phases/
  - 可写: codebase-state/

  **🎯 任务**: 开发后源码重扫描——
  1. 重新扫描项目源码，对比 codebase-state/01-04 开发前快照
  2. 生成 codebase-state/06-post-dev-diff.md（前后差异对比报告）
  3. 🔴 验证 DI 设计规格的覆盖度（设计中的需求是否都已实现）
  4. 更新 codebase-state/codebase-INDEX.md

  **🔴 文件产出强制要求**: 必须 Write → Read 验证 → TASK_COMPLETE
```

**Gate 6 检查**: Read codebase-state/codebase-INDEX.md → 存在 + 含更新的 01-04 + 06-post-dev-diff.md + DI 覆盖度验证？→ 通过则继续
...
- ✅ Gate 2 (Architecture Gate): architecture/arch-INDEX.md — 每任务架构响应完整（完整架构/影响评估/架构签批）
- ✅ Gate 2b (Doc-Validate Gate): doc-state/doc-INDEX.md — 架构文档与 DI 规格一致性校验通过，接口契约验证通过，ADR 追溯完整
- ✅ Gate 3 (TDD Gate): code-state/code-INDEX.md — 全部任务实现，TDD 证据齐全
- ✅ Gate 4 (Verify Gate): test-report/test-INDEX.md — 每任务验收标准逐项通过，有新鲜测试输出
- ✅ Gate 5 (Review Gate): review-report/review-INDEX.md — 0 Critical
- ✅ Gate 6 (Codebase Re-scan): codebase-state/codebase-INDEX.md — 开发后重扫描 + DI 覆盖度验证
- ✅ Gate 7 (Finish Gate): 可交付

## 📦 产出清单
.dev-genius/blackboard/
├── codebase-state/ (含 codebase-INDEX.md + 6 子文件)
├── task-queue/ (含 task-INDEX.md + 子文件)
├── architecture/ (含 arch-INDEX.md + 子文件)
├── doc-state/ (含 doc-INDEX.md + 3 子文件)
├── code-state/ (含 code-INDEX.md + 4 子文件)
├── test-report/ (含 test-INDEX.md + 4 子文件)
└── review-report/ (含 review-INDEX.md + 6 子文件)
...
💡 /loop 10m /dev-genius-coordinator 读取 MASTER-INDEX.md，从 task-queue/task-INDEX.md 取出下一个任务，调用对应专家按七道门控执行
```
...
- [ ] ✅ 包含模式标识：`using Blackboard pattern with Event Bus for state synchronization and mandatory seven-gate pipeline`
...
- [ ] ✅ MCP三级授权机制完整
- [ ] ✅ 七道门控 gate check 完整（每道门有明确的通过标准 + 文件验证路径）
- [ ] ✅ /goal + /loop 集成完整
...
| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| 专家完成但未产出文件 | 专家仅在对话中返回内容未调用Write | 使用产出验证流程：Read检查→重试→协调器兜底写入 |
| Architect 被跳过 | 旧版任务映射表允许跳过 | 所有路径必经 Architect（至少架构签批） |
| QA Tester 被跳过 | 协调器认为 Developer 自测足够 | Gate 4 必经，验证深度由协调器判断 |
...
| F 未产出文件 | codebase-analyst 未写入 codebase-state/ 子文件 | Gate 0/6 检查验证 codebase-INDEX.md 存在性；缺失 → 重试 F |
| F Gate 6 差异对比遗漏 | codebase-analyst 开发后重扫描未生成 06-post-dev-diff.md | Gate 6 检查验证 06-post-dev-diff.md 存在；缺失 → 退回 F 重新扫描 |
