...
**下游**: 最终交付的可运行软件产品
**推荐工作流链**: **DM(Stage 1-7) → DI(Phase 0-12) → DV(Gate 1-6)**。也可 DI→DV，不推荐未经 DI 直接 DV。
...
| **流程顺序** | Gate 1(Plan) → Gate 2(Arch) → Gate 3(TDD) → Gate 4(Verify) → Dev↔QA闭环 → Gate 5(Review) → Gate 6(Finish) |
...
**跨团队变更感知（上游 DI 产出更新）**：
- DI Phase 10 产出的 INDEX.md 含 `最后更新: {ISO8601}` 字段 + 完整文档清单（标准 6 份 + 自定义文档）
- DV 上游检测时读取 INDEX.md 并对比已知时间戳
- 时间戳变更 → 上游有更新 → 重新全读标准文档 + 按需读取 INDEX.md 中引用的自定义文档 → Planner 检查是否影响当前开发
...
开发流程必须经过六道质量门控，**每道门控未通过时禁止进入下一阶段**。Gate 2 (Architecture Gate) 和 Gate 4 (Verify Gate) 是必经门控——协调器不可跳过。
...
| Gate 2 🔴 | architecture.md | 存在 + 含当前任务的架构响应（完整/评估/签批至少其一） | 退回 Architect |
| Gate 3 | code-state.md | 存在 + 含 TDD 证据（RED 失败输出+GREEN 通过输出） | 退回 Developer |
| Gate 4 🔴 | test-report.md | 存在 + 验收标准逐项 ✅/❌ + 有测试输出证据 | 触发 Dev↔QA 闭环 |
| Gate 5 | review-report.md | 存在 + 0 Critical + 问题分级完整 | 退回 Developer 修复 |
| Gate 6 | 全部 5 个黑板文件 | 全部任务完成 + 全部测试通过 + 0 Critical | 不可交付 |
...
| architect | 架构实施师 | 技术架构细化、模块接口设计、ADR 编写、架构签批（三级响应） | Gate 2 必经 | architecture.md | Sonnet |
| developer | 开发工程师 | TDD 功能实现、Bug 修复、系统化调试、stop-on-mismatch 上报 | Gate 3 | code-state.md | Sonnet |
| qa-tester | 测试工程师 | 验收标准验证、测试设计、Bug 报告、回归测试 | Gate 4 必经 | test-report.md | Sonnet |
| analyst | 代码审查师 | 代码评审、安全审计、性能分析、合并前检查 | Gate 5+6 | review-report.md | Sonnet |
...
**Gate 2 检查**：Read architecture.md → 当前任务的架构响应存在（完整架构/影响评估/架构签批至少其一）？→ 通过则继续
...
**Gate 3 检查**: Read code-state.md → 有测试命令和通过结果？→ 通过则继续
...
**Gate 4 检查**: Read test-report.md → 验收标准全部通过？→ 通过则继续。有 Bug？→ 触发 Dev↔QA 闭环
...
**Gate 5 检查**: Read review-report.md → 无 Critical？→ 通过
...
- ✅ Gate 2 (Architecture Gate): architecture.md — 每任务架构响应完整（完整架构/影响评估/架构签批）
- ✅ Gate 3 (TDD Gate): code-state.md — 全部任务实现，TDD 证据齐全
- ✅ Gate 4 (Verify Gate): test-report.md — 每任务验收标准逐项通过，有新鲜测试输出
- ✅ Gate 5 (Review Gate): review-report.md — 0 Critical
- ✅ Gate 6 (Finish Gate): 可交付
