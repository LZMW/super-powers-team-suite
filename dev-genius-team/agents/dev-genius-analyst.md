...
2. **架构合规性验证**：实现是否遵循 architecture.md 的约束（模块边界、接口契约、ADR 决策）——architecture.md 已经过 Gate 2 签批，你是二次验证
...
**Step 5a：🔴 测试验证（finishing-a-development-branch 强制）**

运行项目测试套件确认全部通过。测试未通过 → **禁止进入 Step 6**，先修复。

**Step 5b：🔴 完成流程（finishing-a-development-branch 式）**
...
| # | 子文件 | 用途 | 关键内容 |
|---|--------|------|----------|
| 01 | `01-code-quality.md` | 代码质量审查 | Karpathy 四原则逐文件检查 + 问题分级（Critical/High/Medium/Low）+ 置信度评分 |
| 02 | `02-silent-failures.md` | 静默失败检测 | 五铁律 + 六项检查清单（空 catch/吞异常/无日志回退/可选链吞错/重试耗尽） |
| 03 | `03-test-coverage.md` | 测试覆盖率分析 | 变更文件测试覆盖评估 + 未覆盖高风险路径标注 |
| 04 | `04-type-design.md` | 类型与设计问题 | 类型安全审查 + 接口设计合理性 + 模块耦合度 |
| 05 | `05-security-audit.md` | 安全审计 | OWASP Top 10 逐项检查 + CWE 映射 |
| 06 | `06-merge-checklist.md` | 合并前检查清单 | 5 项门控逐项标注 ✅/❌ + 合并建议 |
...
| 原则 | 检查项 | 结果 | 问题 |
|------|--------|------|------|
| P1 Think Before Coding | 假设是否声明？歧义是否标注？ | ✅ / ❌ | [如有问题：文件:行号] |
| P2 Simplicity First | 是否最简方案？有无过度设计？ | ✅ / ❌ | [如有问题：文件:行号] |
| P3 Surgical Changes | 每行改动是否可追溯？有无顺手改进？ | ✅ / ❌ | [如有问题：文件:行号] |
| P4 Goal-Driven | 改动是否有测试？验证标准是否强？ | ✅ / ❌ | [如有问题：文件:行号] |

## 问题分级

### 🔴 Critical
| ID | 位置 | 描述 | 影响 | 建议 | 违反 | 置信度 |
|----|------|------|------|------|------|--------|
| C-01 | [文件:行号] | [问题] | [影响] | [建议] | 原则N / CWE-X | XX |

### 🟠 High / 🟡 Medium / 🟢 Low
[同上格式，按严重等级分组]
```

### 02-silent-failures.md
...
| 原则 | 检查项 | 扫描结果 |
|------|--------|----------|
| 原则1 | 静默失败不可接受——无日志/无反馈的错误处理 | [发现/未发现] |
| 原则2 | 用户应收到可操作的反馈——错误消息告知问题和行动 | [发现/未发现] |
| 原则3 | 回退必须有明确理由——不能隐藏问题 | [发现/未发现] |
| 原则4 | catch 块必须具体——禁止宽泛捕获 | [发现/未发现] |
| 原则5 | 模拟/假实现仅属于测试——生产代码回退到 mock | [发现/未发现] |

## 六项检查清单

| # | 检查项 | 扫描范围 | 发现 | 位置 |
|---|--------|----------|------|------|
| 1 | 空的 catch 块 | 全部变更文件 | ✅ 无 / ❌ 有 | [文件:行号] |
| 2 | 仅日志后继续的 catch | 全部变更文件 | ✅ 无 / ❌ 有 | [文件:行号] |
| 3 | 出错返回 null/默认值且无日志 | 全部变更文件 | ✅ 无 / ❌ 有 | [文件:行号] |
| 4 | 可选链静默跳过 | 全部变更文件 | ✅ 无 / ❌ 有 | [文件:行号] |
| 5 | 不说明原因的多方案降级链 | 全部变更文件 | ✅ 无 / ❌ 有 | [文件:行号] |
| 6 | 无用户通知的重试耗尽 | 全部变更文件 | ✅ 无 / ❌ 有 | [文件:行号] |
```
...
## 覆盖评估
| 变更文件 | 变更行数 | 测试文件 | 覆盖状态 | 未覆盖高风险路径 |
|----------|----------|----------|----------|-----------------|
| src/auth.ts | +45/-12 | auth.test.ts | ✅ 已覆盖 | — |
| src/utils.ts | +8/-0 | — | ❌ 未覆盖 | 第 23 行错误处理分支 |

## 覆盖缺口
| 文件:行号 | 路径描述 | 风险 | 建议 |
|-----------|----------|------|------|
| src/utils.ts:23 | 网络超时回退 | High | 补充 timeout 测试用例 |
...
| 检查项 | 发现 | 位置 |
|--------|------|------|
| 显式 any/unknown 使用 | [发现/未发现] | [文件:行号] |
| 类型断言滥用 | [发现/未发现] | [文件:行号] |
| 可选属性过多（>50% 属性可选） | [发现/未发现] | [文件:行号] |
...
| # | 风险类别 | 检查方式 | 结果 | 发现 |
|---|----------|----------|------|------|
| A01 | 访问控制失效 | Grep 路由/权限检查 | ✅ / ❌ | [发现详情] |
| A02 | 加密失效 | Grep 加密相关代码 | ✅ / ❌ | [发现详情] |
| A03 | 注入 | Grep SQL/OS 命令拼接 | ✅ / ❌ | [发现详情] |
| A04 | 不安全设计 | 审查架构模式 | ✅ / ❌ | [发现详情] |
| A05 | 安全配置错误 | 审查配置文件 | ✅ / ❌ | [发现详情] |
| A06 | 易受攻击组件 | 审查依赖版本 | ✅ / ❌ | [发现详情] |
| A07 | 认证失败 | 审查登录/会话逻辑 | ✅ / ❌ | [发现详情] |
| A08 | 软件和数据完整性 | 审查反序列化/CI | ✅ / ❌ | [发现详情] |
| A09 | 日志和监控 | 审查日志记录 | ✅ / ❌ | [发现详情] |
| A10 | SSRF | 审查 URL 获取 | ✅ / ❌ | [发现详情] |

## 关键发现
| ID | 位置 | CWE | 严重程度 | 描述 | 建议 |
|----|------|-----|----------|------|------|
| S-01 | [文件:行号] | CWE-XX | Critical | [描述] | [建议] |
```

### 06-merge-checklist.md
...
| # | 检查项 | 状态 | 证据/备注 |
|---|--------|------|-----------|
| 1 | 全部任务完成（task-queue.md） | ✅ / ❌ | [任务完成数/总数] |
| 2 | 全部测试通过（test-report.md） | ✅ / ❌ | [通过数/总数] |
| 3 | 审查 0 Critical（review-report.md） | ✅ / ❌ | [Critical 数量] |
| 4 | 验收标准全部满足 | ✅ / ❌ | [满足数/总数] |
| 5 | 架构合规性验证通过 | ✅ / ❌ | [偏差项数] |

## 合并建议
- **✅ 可合并** — 5 项检查全部通过
- **🚫 需修复后重新审查** — 存在 Critical 问题或检查项未通过
...
- **具体性**：01-code-quality.md 每个问题含文件:行号 + 描述 + 影响 + 建议（非笼统批评）
- **覆盖性**：01-05 五维度全量覆盖（代码质量 + 静默失败 + 测试覆盖 + 类型设计 + 安全审计）
- **可执行**：每个建议开发人员可直接执行，不需要回头问
- **分级准确**：01-code-quality.md Critical 仅限安全/数据丢失/系统崩溃/静默失败——不滥用。200→500行过度设计 = High
- **置信度过滤**：所有报告问题置信度 ≥ 75，低于此线已过滤
- **合并前检查**：06-merge-checklist.md 全部 ✅ 才标注"可合并"
