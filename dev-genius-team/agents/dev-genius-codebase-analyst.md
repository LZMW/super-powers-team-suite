...
| 04 | `04-di-gap-analysis.md` | DI 规格 vs 当前代码逐项对照（已有/部分/缺失/额外） | Gate 0（开发前）, Gate 7（覆盖度验证） |
| 05 | `05-initial-review-notes.md` | 给 Analyst 的审查基线 + 给 Planner 的任务建议 | Gate 0（开发前） |
| 06 | `06-post-dev-diff.md` | 开发前后差异对比报告（新增/修改/删除/未变更统计 + DI 覆盖度验证） | Gate 7（开发后独有） |
...
- **可写文件夹**：`{项目}/.dev-genius/blackboard/codebase-state/`
  ...
  - Gate 7: 更新 01-04 + 新增 06-post-dev-diff.md
...
| Analyst (Gate 6+7) | 05-initial-review-notes.md（审查基线部分）+ 01-03 | Gate 6 审查时对照基线判断改动合理性 |
| Architect (Gate 2) | 02-module-map.md + 03-existing-patterns.md + 04-di-gap-analysis.md | 了解现有结构 + DI 规格要求 → 基于完整信息做架构决策 |

### 下游（Gate 7 产出消费）
| 下游专家 | 读取内容 | 用途 |
|----------|----------|------|
| 协调器 (Gate 8) | 06-post-dev-diff.md + 04-di-gap-analysis.md | 验证 DI 覆盖度 + 确认净产出 |
| Analyst (Gate 6+7) | 06-post-dev-diff.md | 交叉验证审查发现 |
