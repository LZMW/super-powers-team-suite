...
| # | 子文件 | 用途 | 关键内容 |
|---|--------|------|----------|
| 01 | `01-acceptance-verification.md` | 验收标准逐项验证 | 每个标准标注 ✅/❌ + 新鲜测试命令输出证据 |
| 02 | `02-test-cases.md` | 测试用例清单 | 正常流程 + 边界条件 + 异常场景三层覆盖 |
| 03 | `03-regression-results.md` | 回归测试结果 | 修复后验证：原 Bug 不复现 + 关联功能不受影响 |
| 04 | `04-bug-reports.md` | Bug 报告清单 | 可复现格式（环境/步骤/预期/实际/严重程度） |

### 01-acceptance-verification.md

```markdown
# 验收标准验证 — 任务 #N: [标题]

> gen: gen-{N} | 验证时间: {ISO8601}

## 📊 验证统计
- 验收标准总数: N | 通过: X | 失败: Y | 无法验证: Z

## 逐项验证
| # | 验收标准 | 来源 | 结果 | 测试命令 | 输出摘要 |
|---|----------|------|------|----------|----------|
| AC-01 | [标准描述] | task-queue.md T-00X | ✅ | `npm test -- module` | 0 failures, 5 passed |
| AC-02 | [标准描述] | DI valid-INDEX.md §X | ❌ | `npm test -- edge` | 1 failure: expected 200, got 500 |
| AC-03 | [标准描述] | task-queue.md T-00X | ⚠️ | — | 测试环境不支持，已上报协调器 |

## 证据要求
- 每条 ✅/❌ 附带本次运行的测试命令和输出（新鲜执行，非缓存）
- 禁止: "应该通过"、"看起来正确"、依赖上次运行结果
```
...
> gen: gen-{N} | 设计时间: {ISO8601}

## 测试统计
- 总用例: N | 正常流程: X | 边界条件: Y | 异常场景: Z
- 通过: P | 失败: F | 通过率: P/N%
...
## 边界条件用例
### TC-0XX: [用例标题]
[同上格式，覆盖空值/最大最小值/特殊字符/并发/超时]

## 异常场景用例
### TC-0XX: [用例标题]
[同上格式，覆盖网络失败/超时/权限不足/数据无效]

## 负面空间映射清单
| 应覆盖项 | 来源 | 是否已覆盖 |
|----------|------|------------|
| 登录密码为空 | code-state.md 分支 | ✅ TC-0XX |
| 网络超时 | 异常路径 | ⚠️ 未覆盖（环境限制） |
```

### 03-regression-results.md

```markdown
# 回归测试结果 — 任务 #N: [标题]

> gen: gen-{N} | 执行时间: {ISO8601}
...
| # | 测试用例 | 类型 | 结果 | 证据 |
|---|----------|------|------|------|
| 1 | 原 Bug 复现场景 | Bug 修复验证 | ✅ 已修复 | `test command` → 0 failures |
| 2 | 关联功能 A | 回归测试 | ✅ 正常 | `test command` → 0 failures |
| 3 | 关联功能 B | 回归测试 | ✅ 正常 | `test command` → 0 failures |
...
## Bug 统计
- P0 阻塞: X | P1 严重: Y | P2 一般: Z | P3 建议: W
...
- **验证证据**：01-acceptance-verification.md 每个验收标准有测试命令输出作为证据（非口头声称）
- **三层覆盖**：02-test-cases.md 正常流程 + 边界条件 + 异常场景，每层至少 1 个用例
- **Bug 可复现**：04-bug-reports.md 每个 Bug 含完整复现步骤（环境+步骤+预期+实际）
...
- **MCP 工具**（需协调器授权）：🔴 首次必检 codegraph_status——未初始化立即汇报「请执行 codegraph init -i」。CodeGraph 代码分析工具集（10 个，🟢 可选级——跨文件追溯测试证据链）
